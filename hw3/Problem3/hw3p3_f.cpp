...
int main()
{
   ...
   U_CHAR *dataO, *data, *data2, color_table[1024];
   int i, j, k, temp;
   ...
   //
   dataO = (U_CHAR *)malloc( ((biWidth*1 +3)/4 *4)*biHeight*1 );
   if (data == NULL) {
   ...
   FREAD(input_file,dataO,((biWidth*1 +3)/4 *4)*biHeight*1);

   fclose (input_file);
	//open c
   if ((input_file = fopen("hw3p3_c.bmp", "rb")) == NULL)
   {
       fprintf(stderr, "File can't open.\n");
       exit(0);
   }
   ...
   fclose (input_file);
	 ...
	 //open e
   if ((input_file = fopen("hw3p3_e.bmp", "rb")) == NULL)
   {
       fprintf(stderr, "File can't open.\n");
       exit(0);
   }
   ...
   //
   data2 = (U_CHAR *)malloc( ((biWidth*1 +3)/4 *4)*biHeight*1 );
   if (data2 == NULL) {
      fprintf(stderr,"Insufficient memory.\n");
      fclose (input_file);
      exit(0);
   ...
   //
   fseek(input_file,bfOffBits,SEEK_SET);
   FREAD(input_file,data2,((biWidth*1 +3)/4 *4)*biHeight*1);
	...
	for (i=0; i < biHeight; i++)
   {
       ...
       for (j=0; j < biWidth; j++)
       {
					 data[k] = f(dataO[k]*2 - data[k] - data2[k]);
           k = k+1;
       }
   }
   //
   /* �}�ҷs�ɮ� */
   if( ( output_file = fopen("hw3p3_f.bmp","wb") ) == NULL ){
