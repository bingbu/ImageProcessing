...
int main()
{
   ...
   for (i=0; i < biHeight; i++)
   {
       ...
       for (j=0; j < biWidth; j++)
       {
					Ri=(int)(cos(2*acos(0)*21/180)*(i-biHeight/2)-sin(2*acos(0)*21/180)*(j-biWidth/2)+(biHeight/2));
	      	Rj=(int)(cos(2*acos(0)*21/180)*(j-biWidth/2)+sin(2*acos(0)*21/180)*(i-biHeight/2)+(biWidth/2));

					if (Ri >= 0 && Ri < biHeight && Rj >= 0 && Rj < biWidth)
			 			data2[k] = data1[Rj+Ri*biWidth4];
           k = k+1;
       }
   }
   ...
   //
   /* �}�ҷs�ɮ� */
   if( ( output_file = fopen("P3.bmp","wb") ) == NULL ){
