...
int main()
{
   ...
   for (i=0; i < biHeight; i++)
   {
       ...
       for (j=0; j < biWidth; j++)
       {
					Ri = (j-biWidth/2) * cos(DEGREE) - (i-biHeight/2) * sin(DEGREE) + biHeight/2;
					Rj = (j-biWidth/2) * sin(DEGREE) + (i-biHeight/2) * cos(DEGREE) + biWidth/2;
					Rk = Ri* ((biWidth*1 +3)/4 *4) + Rj;
					//printf("Rk: %d\n", Rk);
					data2[Rk] = data1[k];
           k = k+1;
       }
