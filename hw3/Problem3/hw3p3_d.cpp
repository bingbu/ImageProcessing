...
int main()
{
   ...
   for (i=1; i < biHeight-1; i++)
   {
       ...
       {
					 new_data[k] = abs(data[k+biWidth-1]+data[k+biWidth]*2+data[k+biWidth+1]-data[k-biWidth-1]-data[k-biWidth]*2-data[k-biWidth+1])
					 + abs(data[k-biWidth+1]+data[k+1]*2+data[k+biWidth+1]-data[k-biWidth-1]-data[k-1]*2-data[k+biWidth-1]);
   ...
   //
   /* �}�ҷs�ɮ� */
   if( ( output_file = fopen("hw3p3_d.bmp","wb") ) == NULL ){
