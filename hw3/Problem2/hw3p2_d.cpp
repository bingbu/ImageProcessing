...
int main()
{
   ...
   for (i=1; i < biHeight-1; i++)
   {
       ...
       {
					 new_data[k] = f(data[k]*9 - data[k-biWidth-1] - data[k-biWidth] - data[k-biWidth+1] - data[k-1] - data[k+1] - data[k+biWidth-1] - data[k+biWidth] - data[k+biWidth+1]);
   ...
   //
   /* �}�ҷs�ɮ� */
   if( ( output_file = fopen("hw3p2_d.bmp","wb") ) == NULL ){
