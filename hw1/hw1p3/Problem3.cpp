...
int main()
{
   ...
   for (i=0; i < biHeight; i++)
   {
     ...
     for (j=0; j < biWidth; j++)
     {
				int gap = pow(2.0, 9-Lv);
				
				if(data1[k]<gap)
					data2[k]=0;
				for(LvCount=gap; LvCount<pow(2.0, 8); LvCount+=gap)
				{
					if(data1[k]>=LvCount && data1[k]<=LvCount+gap)
						data2[k] = LvCount+gap-1;
				}
   ...
   //
   /* 開啟新檔案 */
   if( ( output_file = fopen("128_level.bmp","wb") ) == NULL ){
