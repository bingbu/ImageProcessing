...
int main()
{
   ...
   /* �}���ɮ� */
   if( ( input_file = fopen("p1_sub.bmp","rb") ) == NULL ){
   ...
   for (i=0; i < biHeight; i++)
   {
		...
		for (j=0; j < biWidth-1; j++)
		{
	 		for (i1 = i*RATIO; i1 < i*RATIO+RATIO; i1++)
				...
				new_data[i1*biWidth*RATIO + j1] = (data[k + 1] - data[k]) / 4 * (j1 - j*RATIO) + (data[k + biWidth] - data[k]) / 4 * (i1 - i*RATIO) + (data[k + biWidth + 1] - data[k + 1] + data[k] - data[k + biWidth]) / 16 * (i1 - i*RATIO) *(j1 - j*RATIO) + data[k];
   ...
   //
   /* �}�ҷs�ɮ� */
   if( ( output_file = fopen("p1_re.bmp","wb") ) == NULL ){
