...
int main()
{
   ...
   i = ReadDataSize("Fig3.08(a).bmp");
   data1 = (U_CHAR *)malloc( i );
   if (data1 == NULL) {
      exit(0);
   }
  
   ReadImageData("Fig3.08(a).bmp", bmpfileheader1, bmpinfoheader1, color_table1, data1);
   biWidth           =   GET_4B(bmpinfoheader1,4);
   biHeight          =   GET_4B(bmpinfoheader1,8);

   //
   i = ReadDataSize("new2(1).bmp");
   data2 = (U_CHAR *)malloc( i );
   if (data2 == NULL) {
      exit(0);
   }

   ReadImageData("new2(1).bmp", bmpfileheader1, bmpinfoheader1, color_table1, data2);
   ...
   for (i=0; i < biHeight; i++)
   {
       ...
       for (j=0; j < biWidth; j++)
       {
		   ...
		   data1[k] = (data1[k] < 0)? 0:data1[k];
		   data1[k] = (data1[k] > 255)? 255:data1[k];
   ...
   //
   /* �}�ҷs�ɮ� */
   if( ( output_file = fopen("new(1).bmp","wb") ) == NULL ){
